...
let maxNumber = 10; //한 파티당 최대 인원 수 입니다. 이는 의뢰자의 조건에 의하여 파티마다 가변으로 제공되지 않고 있으며, 설정으로만 일괄 제공됩니다. 수정시 정상 작동을 보장하지 않으며, 인챗에서 수정시 원정이 종료됩니다.
let inChatSettingPermissin = 1; //인챗에서 설정을 변경할 수 있도록 할 것인지 설정합니다.
let partyDeletePermission = 0; //파티 삭제 권한 관리입니다.
let rightToCancel = 0; //강제 참가 취소 관리 권한입니다.
let addGuestPermissin = 0; //나 이외의 사람을 참여시킵니다. 본 기능을 이용할 경우 강제로 참여한 사람의 이름이 다르면 강제로 취소해야 합니다.
let partyNameEditPermission = 0; //파티 이름을 수정할 권한을 정합니다.
